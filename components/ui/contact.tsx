...
import { contactContent as defaultContact, websiteInfo, socialLinks as defaultSocials } from "@/data";
import {
  validateName,
  validateEmail,
  validateMessage,
...
export function Contact() {
    ...
    const fetchData = async () => {
      try {
        const [contactRes, socialsRes] = await Promise.all([
          fetch("/api/admin/files?file=contact.json", { cache: 'no-store' }),
          fetch("/api/admin/files?file=socials.json", { cache: 'no-store' })
        ]);

        const contactData = await contactRes.json();
        const socialsData = await socialsRes.json();

        if (contactData && !contactData.error) setContent(contactData);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMessage(error.message || content.formStatus.error);
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a
                      ...
                      className="text-white hover:text-accentColors-purple transition-colors"
                    >
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a
                      href={`tel:${content.contactInfo.phone.value.replace(
                        /\s/g,
                        ""
                      )}`}
                      className="text-white hover:text-accentColors-purple transition-colors"
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-zinc-400 text-sm">
                      {content.contactInfo.location.label}
                    </p>
                    <p className="text-white">
                      {content.contactInfo.location.value}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <div className="flex gap-4">
                {socials.map((link, index) => {
                  ...
                  else if (link.icon === "IconBrandLinkedin")
                    Icon = IconBrandLinkedin;
                  else if (link.icon === "IconBrandFacebook")
          ...
          >
            ...
            <div className="relative">
              ...
              <form
                ...
                className="bg-zinc-800/30 rounded-xl p-6 border border-zinc-700/50 relative z-10"
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  >
                    <label
                      htmlFor="name"
                      className="block text-zinc-400 text-sm mb-2 flex items-center gap-2"
                    >
                    ...
                    <input
                      ...
                      className={`w-full bg-zinc-800 border ${
                        touched.name && errors.name
                          ...
                          ? "border-green-500"
                          : "border-zinc-700"
                      } rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-accentColors-primary/50 focus:border-accentColors-primary transition-all duration-300 shadow-sm hover:shadow-accentColors-primary/10`}
                    />
                    {touched.name && errors.name && (
                      <motion.p
                    ...
                    <input
                      ...
                      className={`w-full bg-zinc-800 border ${
                        touched.email && errors.email
                          ...
                          ? "border-green-500"
                          : "border-zinc-700"
                      } rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-accentColors-accent/50 focus:border-accentColors-accent transition-all duration-300 shadow-sm hover:shadow-accentColors-accent/10`}
                    />
                    {touched.email && errors.email && (
                      <motion.p
                        initial={{ opacity: 0, y: -10 }}
                  ...
                  <textarea
                    ...
                    className={`w-full bg-zinc-800 border ${
                      touched.message && errors.message
                        ...
                        ? "border-green-500"
                        : "border-zinc-700"
                    } rounded-lg px-4 py-3 text-white min-h-[150px] focus:outline-none focus:ring-2 focus:ring-accentColors-highlight/50 focus:border-accentColors-highlight transition-all duration-300 shadow-sm hover:shadow-accentColors-highlight/10 resize-none`}
                  />
                  {touched.message && errors.message && (
                    <motion.p
                      initial={{ opacity: 0, y: -10 }}
                ...
                >
                  <div className="flex flex-col items-end gap-2 w-full">
                    ...
                    <div className="flex justify-end">
                      <motion.button
                        ...
                        className={`px-6 py-3 bg-accentColors-primary hover:bg-accentColors-highlight text-white rounded-lg shadow-md shadow-accentColors-primary/20 ${
                          isSubmitting
                            ? "opacity-70 cursor-not-allowed"
                            : "hover:scale-105"
                        } focus:outline-none focus:ring-2 focus:ring-accentColors-primary focus:ring-opacity-50 transition-all duration-300 inline-flex items-center gap-2`}
                  ...
                  {submitStatus === "error" && (
                    <motion.div
                      ...
                      role="alert"
                    >
                      <p>{errorMessage || content.formStatus.error}</p>
